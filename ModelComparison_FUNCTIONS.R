  ...
  if(draw) {
    ...
    if(npars > 1) {
      ...
      for(i in 1:ncol(inds)) {
        ...
        contour(pgrid[[inds[1, i]]], pgrid[[inds[2, i]]], zgrid, add = TRUE, col = "red")
      }
    } else {
      ## plot
      hist(samples[, 1], xlab = colnames(samples)[1], ylab = "Density", freq = FALSE)
      lines(pgrid[[1]], dnorm(pgrid[[1]], mean = impmean, sd = sqrt(impcov)), col = "red")
    }
  }
