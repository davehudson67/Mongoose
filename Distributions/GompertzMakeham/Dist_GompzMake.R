...
dGompzMake <- function(x, a, b, c, zL = NA, zU = NA, log = FALSE) {
    ...
    logS[zN_ind] <- -c[zN_ind] * x[zN_ind] - (a[zN_ind] / b[zN_ind]) * (exp(b[zN_ind] * x[zN_ind]) - 1)
    logH[zN_ind] <- log(a[zN_ind] * exp(b[zN_ind] * x[zN_ind]) + c[zN_ind])
    logProb <- logH + logS
  
    #interval truncation
    zI_ind <- which(!is.na(zL) & !is.na(zU) & checkPass[, 1] & checkPass[, 2])
    if(length(zI_ind) > 0) {
        logS_zL <- -c[zI_ind] * zL[zI_ind] - (a[zI_ind] / b[zI_ind]) * (exp(b[zI_ind] * zL[zI_ind]) - 1)
        logS_zU <- -c[zI_ind] * zU[zI_ind] - (a[zI_ind] / b[zI_ind]) * (exp(b[zI_ind] * zU[zI_ind]) - 1)
        S_zL <-  exp(logS_zL)
        S_zU <- exp(logS_zU)
    ...
    if(length(zL_ind) > 0) {
    logS_zL <- -c[zL_ind] * zL[zL_ind] - (a[zL_ind] / b[zL_ind]) * (exp(b[zL_ind] * zL[zL_ind]) - 1)
    ...
    if(length(zU_ind) > 0) {
        logS_zU <- -c[zU_ind] * zU[zU_ind] - (a[zU_ind] / b[zU_ind]) * (exp(b[zU_ind] * zU[zU_ind]) - 1)
...
pGompzMake <- function(q, a, b, c, zL = NA, zU = NA,
...
## Quantile function
qGompzMake<- function(p, a, b, c, zL = NA, zU = NA,
                     lower.tail = TRUE, log.p = FALSE) {
    ## all parameters length = 1 or length(x)
    ntot <- length(p)
    if ((length(a) != 1 & length(a) != ntot) | 
    ...
    if(length(whichPass) > 0) {
        ...
        w0 <- (a / c) * exp(a / c) * ((1 - p)^(-b / c))
        w0 <- lambertW0(w0)
        out1 <- (a / (b * c)) - (1 / c) * log(1 - p) - (1 / b) *  w0
        ...
        if(length(zI_ind) > 0) {
            ...
            w0[zI_ind] <- (a[zI_ind] / c[zI_ind]) * exp(a[zI_ind] / c[zI_ind]) * ((1 - p[zI_ind])^(-b[zI_ind] / c[zI_ind]))
            w0[zI_ind] <- lambertW0(w0[zI_ind])
            out1[zI_ind] <- (a[zI_ind] / (b[zI_ind] * c[zI_ind])) - (1 / c[zI_ind]) * log(1 - p[zI_ind]) - (1 / b[zI_ind]) *  w0[zI_ind]
        }
        ...
        if(length(zL_ind) > 0) {
            ...
            w0[zL_ind] <- (a[zL_ind] / c[zL_ind]) * exp(a[zL_ind] / c[zL_ind]) * ((1 - p[zL_ind])^(-b[zL_ind] / c[zL_ind]))
            w0[zL_ind] <- lambertW0(w0[zL_ind])
            out1[zL_ind] <- (a[zL_ind] / (b[zL_ind] * c[zL_ind])) - (1 / c[zL_ind]) * log(1 - p[zL_ind]) - (1 / b[zL_ind]) *  w0[zL_ind]
        }
        ...
        if(length(zU_ind) > 0) {
            ...
            w0[zU_ind] <- (a[zU_ind] / c[zU_ind]) * exp(a[zU_ind] / c[zU_ind]) * ((1 - p[zU_ind])^(-b[zU_ind] / c[zU_ind]))
            w0[zU_ind] <- lambertW0(w0[zU_ind])
            out1[zU_ind] <- (a[zU_ind] / (b[zU_ind] * c[zU_ind])) - (1 / c[zU_ind]) * log(1 - p[zU_ind]) - (1 / b[zU_ind]) *  w0[zU_ind]
        }
        out <- rep(NA, ntot)
        out[whichPass] <- out1
    } else {
...
## Random sampler
rGompzMake <- function(n, a, b, c, zL = NA, zU = NA) {
    ...
    if(length(zN_ind) > 0) {
        w0[zN_ind] <- (a[zN_ind] / c[zN_ind]) * exp(a[zN_ind] / c[zN_ind]) * ((1 - u[zN_ind])^(-b[zN_ind] / c[zN_ind]))
        w0[zN_ind] <- lambertW0(w0[zN_ind])
        rs[zN_ind] <- (a[zN_ind] / (b[zN_ind] * c[zN_ind])) - (1 / c[zN_ind]) * log(1 - u[zN_ind]) - (1 / b[zN_ind]) *  w0[zN_ind]
    } else {
        return(rs)
    }
    ...
    if(length(zI_ind) > 0) {
        ...
        w0[zI_ind] <- (a[zI_ind] / c[zI_ind]) * exp(a[zI_ind] / c[zI_ind]) * ((1 - u[zI_ind])^(-b[zI_ind] / c[zI_ind]))
        w0[zI_ind] <- lambertW0(w0[zI_ind])
        rs[zI_ind] <- (a[zI_ind] / (b[zI_ind] * c[zI_ind])) - (1 / c[zI_ind]) * log(1 - u[zI_ind]) - (1 / b[zI_ind]) *  w0[zI_ind]
    }
    ...
    if(length(zL_ind) > 0) {
        ...
        w0[zL_ind] <- (a[zL_ind] / c[zL_ind]) * exp(a[zL_ind] / c[zL_ind]) * ((1 - u[zL_ind])^(-b[zL_ind] / c[zL_ind]))
        w0[zL_ind] <- lambertW0(w0[zL_ind])
        rs[zL_ind] <- (a[zL_ind] / (b[zL_ind] * c[zL_ind])) - (1 / c[zL_ind]) * log(1 - u[zL_ind]) - (1 / b[zL_ind]) *  w0[zL_ind]
    }
    ...
    if(length(zU_ind) > 0) {
        ...
        w0[zU_ind] <- (a[zU_ind] / c[zU_ind]) * exp(a[zU_ind] / c[zU_ind]) * ((1 - u[zU_ind])^(-b[zU_ind] / c[zU_ind]))
        w0[zU_ind] <- lambertW0(w0[zU_ind])
        rs[zU_ind] <- (a[zU_ind] / (b[zU_ind] * c[zU_ind])) - (1 / c[zU_ind]) * log(1 - u[zU_ind]) - (1 / b[zU_ind]) *  w0[zU_ind]
    }
