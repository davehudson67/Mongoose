...
optimiseNim <- nimbleRcall(function(interval = double(1), 
                                    ...
                                    c = double(0),p = double(0)){}, Rfun = "optimiseRNim",
                           returnType = double(0))

## R function to run optimiser
optimiseRNim <- function(interval, a1, a2, b1, b2, c, p) {
  silerRootFn <- function(x, a1, a2, b1, b2, c, p) {
    abs(pSilerNim(x, a1, a2, b1, b2, c) - p)
  }
  optimise(silerRootFn, interval, a1, a2, b1, b2, c, p)$minimum
}
...
qsilerNim <- nimbleFunction(
  run = function(p = double(0), a1 = double(0),
    ...
    ## run optimiser
    q <- optimiseNim(c(0,qU), a1 = a1, a2 = a2, b1 = b1, b2 = b2, c = c, p = p)
    return(q)
  })
...
registerDistributions(list(
  dsilerNim = list(
    BUGSdist = "dsilerNim(a1, a2, b1, b2, c)",
    Rdist = "dsilerNim(a1, a2, b1, b2, c)",
    pqAvail = TRUE, 
    range = c(0, Inf)
  )
))
